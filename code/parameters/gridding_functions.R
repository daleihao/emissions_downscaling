...
grid_one_iso <- function( iso,
  ...
  sub_flag <- proxy_substitution_mapping[ proxy_substitution_mapping$em == em &
                                            ...
                                            proxy_substitution_mapping$iso == iso, 'sub' ]
  if ( length( sub_flag ) == 0 ) { sub_flag <- 0 }
...
grid_one_sector <- function( sector,
  ...
  if ( sector %in% c( 'AWB', 'AGR', 'ENE', 'FRTB', 'GRSB', 'IND', 'PEAT', 'RCO', 'SLV', 'TRA', 'WST' ) ) {
    iso_list <- gridding_emissions_sector$iso
...
get_proxy <- function( em, year, sector, proxy_mapping, proxy_type = 'primary' ) {
  ...
  if ( proxy_type == 'primary' ) {
    ...
    if (em == 'NMVOC') {
      proxy_file  <- proxy_file [which(extract_em == 'VOC')]
    }
    else {
...
sum_monthly_em <- function( fin_grid, em, sector, year, days_in_month, global_grid_area, seasonality_mapping ) {
  ...
  if ( sector == 'AIR' ) {
    monthly_em_list <- lapply( month_list, function( i ) {
      ...
      month_mass_value <- month_mass_value * 0.000000001 # from kg to kt
      out_df <- data.frame( scenario = sce, em = em, sector = sector, year = year, month = i, units = 'Mt', value = month_mass_value, stringsAsFactors = F  )
      } )
  ...
  if ( sector %in% c( 'AGR', 'AWB', 'FRTB', 'GRSB', 'PEAT', 'IND', 'RCO', 'TRA', 'SHP', 'ENE', 'SLV', 'WST' ) ) {
    ...
    monthly_em_list <- lapply( 1 : 12, function( i ) {
      ...
      month_mass_value <- sum( month_mass, na.rm = T )
      month_mass_value <- month_mass_value * 0.000000001 # from kg to Mt
      out_df <- data.frame( scenario = sce, em = em, sector = sector, year = year, month = i, units = 'Mt', value = month_mass_value, stringsAsFactors = F  )
      } )
    monthly_em <- do.call( 'rbind', monthly_em_list )
    }
...
gridding_initialize <- function( grid_resolution = 0.5,
  ...
  supported_years <- c( 2015, seq( 2020, 2100, 10 ) )
  if ( start_year %!in% supported_years ) { stop( 'Unsupported gridding start year' ) }
  if ( end_year %!in% supported_years ) { stop( 'Unsupported gridding end year' ) }
